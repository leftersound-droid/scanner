...
def object_component(theta: float):
    ...
    for c in product(range(-7,8),repeat=4):
        x,y,z,w=(float(v) for v in c); rho=math.hypot(x,y); r0=2.0
        d2=(rho-r0)**2+z*z+w*w; env=math.exp(-0.5*d2/(SIGMA*SIGMA)); ang=math.atan2(y,x)
...
def save_flow(path,flow):
    ...
    np.savez_compressed(path,source_index=np.asarray([q[0] for q in items],dtype=np.int32),direction=np.asarray([q[1] for q in items],dtype=np.uint8),amount=np.asarray([q[2] for q in items],dtype=np.float64))
...
def m3_moment(phi):
    ...
    for c in LOCAL:
        ...
        ang=math.atan2(y,x); q += excess*complex(math.cos(3*ang),math.sin(3*ang)); weight += excess
    amp=abs(q)/(weight+1e-30); phase=(math.atan2(q.imag,q.real)/3.0) if abs(q)>0 else 0.0
...
def local_flow_m3(flow):
    ...
    for (c,di),v in flow.items():
        ...
        x,y,z,w=c
        if x==0 and y==0: continue
        ang=math.atan2(y,x); q += v*complex(math.cos(3*ang),math.sin(3*ang)); total+=v
    amp=abs(q)/(total+1e-30); phase=(math.atan2(q.imag,q.real)/3.0) if abs(q)>0 else 0.0; phase=phase%(2*math.pi/3)
...
def unwrap_m3_phase(degs):
    ...
    for d in degs:
        ...
        candidates=[d+120*k for k in range(-12,13)]; out.append(float(min(candidates,key=lambda x:abs(x-out[-1]))))
    return out
...
def run_case(case):
    ...
    for frame in range(TOTAL_FRAMES):
        ...
        if case['forced_rotation'] and forced:
            theta=math.radians(ROT_STEP_DEG*frame); new=object_component(theta); apply_component(phi,old,new); old=new; imposed_theta_deg=ROT_STEP_DEG*frame
        ...
        before=sum(phi.values()); pre_m=m3_moment(phi)
        ...
        rows.append({'frame':frame,'phase':'history' if frame<TRAIN_FRAMES else 'free_or_control','externally_forced_this_frame':bool(forced and case['forced_rotation']),'imposed_theta_deg':imposed_theta_deg,'m3_pre':pre_m,'m3_post':post_m,'flow_m3':flow_m,'births':diag.births,'live_transfer':float(diag.live_transfer),'conservation_error':float(sum(phi.values())-before)})
        births+=diag.births; prev=nxt
    np.savez_compressed(cdir/'phi_history.npz',phi_input=np.stack(pin),phi_output=np.stack(pout))
    phases=[r['m3_post']['phase_deg_mod_120'] for r in rows]; unwrap=unwrap_m3_phase(phases)
...
def free_phase_stats(rows):
    free=rows[TRAIN_FRAMES:]; p=np.asarray([r['m3_post']['phase_deg_unwrapped_analysis'] for r in free],float); a=np.asarray([r['m3_post']['amplitude'] for r in free],float); f=np.arange(len(p),dtype=float)
    slope=float(np.polyfit(f,p,1)[0]) if len(p)>1 else 0.0; steps=np.diff(p)
    return {'phase_slope_deg_per_frame_analysis':slope,'mean_abs_phase_step_deg':float(np.mean(np.abs(steps))) if len(steps) else 0.0,'signed_mean_phase_step_deg':float(np.mean(steps)) if len(steps) else 0.0,'phase_steps_deg':[float(x) for x in steps],'amplitude_mean':float(np.mean(a)),'amplitude_start':float(a[0]),'amplitude_end':float(a[-1]),'amplitude_retention_end_over_start':float(a[-1]/(a[0]+1e-30))}
...
def main():
    ...
    results={c['name']:run_case(c) for c in CASES}; stats={k:free_phase_stats(v['rows']) for k,v in results.items()}
    summary={'experiment':'rotation_history_free_release','status':'synthetic history-release test; no angular momentum, torque, inertia, damping, charge, EM law or target free-rotation rule injected','operator':'unchanged scanner.self_reflexive_operator.operator_step','question':'after one externally prescribed 360-degree rotation history, does the field state carry an autonomous rotating/periodic m=3 relation after forcing is removed?','important_note':'m=3 object has 120-degree pattern symmetry; 12 history frames at 30 deg/frame are one imposed 360-degree geometric rotation = three pattern cycles','cases':CASES,'parameters':{'train_frames':TRAIN_FRAMES,'free_frames':FREE_FRAMES,'rotation_step_deg':ROT_STEP_DEG,'domain_l1':DOMAIN_L1,'domain_points':len(DOMAIN),'measure_l1':MEASURE_L1},'checks':{'boundary_unreachable':guard,'births':{k:v['births_total'] for k,v in results.items()}},'free_phase_stats':stats,'raw_phase_series':{k:[r['m3_post'] for r in v['rows']] for k,v in results.items()},'journal':'complete phi input/output, sparse flow, imposed-history metadata, m3 scalar-field moment and flow moment every frame/case'}
