...
DOMAIN_L1=26; SUPPORT_L1=10; FRAMES=8; MEASURE_L1=8; REF_AMP=0.01
DS=directions(DIM)
...
def sector_state(phi,prev,next_flow=None):
    ...
    return {'D3':d3,'D4':d4,'P3':p3,'P4':p4,'A3':a3/max(ad,1),'A4':a4/max(ad,1),'B3':b3/max(bd,1),'B4':b4/max(bd,1),'J3':j3,'J4':j4}


def candidate_values(x,y):
    eps=1e-30
    return {'sum':x+y,'euclidean_norm':math.sqrt(x*x+y*y),'abs_difference':abs(x-y),'product':x*y,'ratio_3_over_4':x/(abs(y)+eps),'ratio_4_over_3':y/(abs(x)+eps),'quadratic_difference':y*y-x*x}

PAIR_DEFS={'D':('D3','D4'),'P':('P3','P4'),'A':('A3','A4'),'B':('B3','B4'),'J':('J3','J4')}
    ...
    for pair in PAIR_DEFS:
        for form in candidate_values(1.0,1.0):
            ...
            for f in range(1,FRAMES):
                ...
                if np.std(arr)>0 and np.std(cs)>0: trends.append(float(np.corrcoef(np.asarray(cs,float),arr)[0,1]))
...
def graph_vectors(results):
    ...
    for c in CADENCES:
        ...
        for row in results[str(c)]['rows']:
            ...
            ev=np.asarray([vn[idx[a]]*vn[idx[b]] for a,b in edges],float); vecs.append(np.concatenate([vn,ev]))
...
def main():
    ...
    if not all(results[str(c)]['boundary_unreachable'] for c in CADENCES):
        raise RuntimeError('invalid causal guard: at least one cadence can reach the boundary')
    ...
    summary={'experiment':'spacetime_inertial_r3_control','status':'synthetic inertial-control scan; no physical velocity, time, length, Lorentz law, c, mass or metric signature injected or claimed','operator':'unchanged scanner.self_reflexive_operator.operator_step','control':'same m=3 ring, same total excess potential, same isotropic relational background; only constant +x translation cadence changes','cadences':CADENCES,'definitions':{'cadence':'lattice-coordinate center shift per frame, analysis/control parameter only','r3_sector':'x/y/z directions','r4_sector':'w directions','ratio_scan':'R3/R4 and R4/R3 are scanned alongside sum, norm, difference, product and quadratic difference; no ratio is privileged','graph':'dimensionless node composition plus explicit operator-dependency edge products; Scanner measurement only'},'parameters':{'frames':FRAMES,'domain_points':len(DOMAIN),'measure_l1':MEASURE_L1,'object_total_excess':OBJ_TOTAL,'reference_amplitude':REF_AMP,'domain_l1':DOMAIN_L1},'checks':{str(c):{'births':results[str(c)]['births_total'],'boundary_unreachable':results[str(c)]['boundary_unreachable']} for c in CADENCES},'candidate_scan':scan,'top_nondefinitional_candidates':scan[:20],'graph_dispersion_across_cadences':graph_dispersion(g),'journal':'complete phi input/output and sparse flows for every frame/cadence plus raw R3/R4 state and candidate values'}
