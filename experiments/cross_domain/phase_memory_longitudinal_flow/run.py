...
def impose_longitudinal_flow(prev,frame,on):
    ...
    amp=FLOW_AMP*abs(s)
    if amp==0.0: return merged,0.0,0
    di=7 if s>0.0 else 6  # +w / -w
    count=0
    for c in DOMAIN:
        target=(c[0],c[1],c[2],c[3]+(1 if di==7 else -1))
...
def sample_phi(phi):
    return np.asarray([phi[c] for c in MEASURE],dtype=np.float64)


def derivative_reconstruction(hist):
    flat=np.asarray(hist,dtype=np.float64)
    diffs=[flat]
    for _ in range(MAX_DERIV): diffs.append(np.diff(diffs[-1],axis=0))
    idx=np.arange(flat.shape[1]); train=(idx%5)!=0; test=~train
    out={}
    for k in range(MAX_DERIV+1):
        Xs=[]; ys=[]
        for t in range(k,len(flat)-1):
            blocks=[flat[t]]+[diffs[o][t-o] for o in range(1,k+1)]
            Xs.append(np.stack(blocks,axis=1)); ys.append(flat[t+1])
        if not Xs: out[str(k)]={'available':False}; continue
        X=np.concatenate(Xs); y=np.concatenate(ys)
        tr=np.tile(train,len(Xs)); te=np.tile(test,len(Xs))
...
def run_case(spec):
    phi={c:BACKGROUND for c in DOMAIN}; prev={}; old_wave={}
    hist=[]; rows=[]; births=0
    for frame in range(FRAMES):
        theta=math.radians(spec['step_deg']*frame)
        nw=wave_component(spec['m'],theta)
        apply_component(phi,old_wave,nw); old_wave=nw
        hist.append(sample_phi(phi))
        op_prev,flow_amp,flow_edges=impose_longitudinal_flow(prev,frame,spec['flow_on'])
        before=sum(phi.values())
        phi,next_flow,diag=operator_step(phi,op_prev,dimension=DIM)
        births+=diag.births
        b=np.asarray(diag.beta_samples,dtype=float)
        ...
        prev=next_flow
    # terminal imposed object state at its cycle-equivalent orientation, before operator
    theta=math.radians(spec['step_deg']*FRAMES)
    nw=wave_component(spec['m'],theta); apply_component(phi,old_wave,nw); hist.append(sample_phi(phi))
    closure=float(np.sqrt(np.mean((hist[-1]-hist[0])**2)))
    return {'case':spec,'frames':FRAMES,'births_total':births,'boundary_unreachable':(5+FRAMES)<DOMAIN_L1,'cycle_closure_rmse_pre_operator':closure,'rows':rows,'derivative_reconstruction_phi':derivative_reconstruction(hist)}
...
def main():
    ...
    result={'experiment':'phase_memory_longitudinal_flow','status':'synthetic flow-input test; no physical emergence claim','definitions':{
      ...
      'fixed_domain':'all points with L1 norm <=19 pre-exist; no boundary rule'
    },'parameters':{'flow_period_frames':FLOW_PERIOD,'flow_amplitude':FLOW_AMP,'frames':FRAMES,'domain_l1_radius':DOMAIN_L1,'domain_points':len(DOMAIN)},'cases':{}}
    for s in CASES: result['cases'][s['name']]=run_case(s)
    (OUT/'summary.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
    np.savez_compressed(OUT/'measurement_coordinates.npz',coord=np.asarray(MEASURE,dtype=np.int16))
